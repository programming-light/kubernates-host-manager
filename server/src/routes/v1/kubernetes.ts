...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      version = (versionInfo.body as any).gitVersion || 'unknown';
    } catch (e) {
...
router.post('/create-namespace', authMiddleware, async (req: Request, res: Response) => {
  try {
    const { name } = req.body;
    if (!name) return res.status(400).json({ error: 'Bad Request', message: 'Namespace name required' });

    const k8sConfig = await k8sConfigManager.loadConfig();
    if (!k8sConfig.connected) return res.status(503).json({ error: 'Service Unavailable' });
...
router.post('/create-deployment', authMiddleware, async (req: Request, res: Response) => {
  try {
...
router.post('/create-service', authMiddleware, async (req: Request, res: Response) => {
  try {
...
router.delete('/delete-resource', authMiddleware, async (req: Request, res: Response) => {
  try {
