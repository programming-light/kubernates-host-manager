...
async function createSSLCommerzPayment(amount, currency, userId, planId) {
...
async function createStripePayment(amount, currency, userId, planId) {
...
export default async function (router) {
    router.post('/initiate', { preHandler: [authMiddleware] }, async (request, reply) => {
        try {
    ...
    router.get('/subscription', { preHandler: [authMiddleware] }, async (request, reply) => {
        try {
            const userId = request.userId;
            ...
            if (!subscription)
                return reply.send({ status: 'no_subscription', plan: null });
    ...
    router.post('/subscription/cancel', { preHandler: [authMiddleware] }, async (request, reply) => {
        try {
            const userId = request.userId;
    ...
    router.get('/resources', { preHandler: [authMiddleware] }, async (request, reply) => {
        try {
            const userId = request.userId;
    ...
    router.post('/webhook', async (request, reply) => {
        try {
            const { tran_id, status } = request.body;
    ...
    router.get('/history', { preHandler: [authMiddleware] }, async (request, reply) => {
        try {
            const userId = request.userId;
