...
router.get('/pods', async (req, res) => {
    try {
        ...
        if (allNamespaces === 'true') {
            ...
            response = await k8sApi.listNamespacedPod(namespace || config.namespace);
        }
...
router.get('/services', async (req, res) => {
    try {
        ...
        const k8sApi = config.kubeConfig.makeApiClient(CoreV1Api);
        const response = await k8sApi.listNamespacedService(namespace || config.namespace);
...
router.get('/deployments', async (req, res) => {
    try {
        ...
        const response = await k8sApi.listNamespacedDeployment(namespace || config.namespace);
...
router.get('/ingresses', async (req, res) => {
    try {
        ...
        const response = await networkingApi.listNamespacedIngress(namespace || config.namespace);
...
router.post('/create-namespace', async (req, res) => {
    try {
        const config = await k8sConfigManager.loadConfig();
...
router.delete('/delete-resource', async (req, res) => {
    try {
        const config = await k8sConfigManager.loadConfig();
        ...
        switch (type) {
            case 'namespace':
                await config.kubeConfig.makeApiClient(CoreV1Api).deleteNamespace(name);
                break;
            case 'deployment':
                await config.kubeConfig.makeApiClient(AppsV1Api).deleteNamespacedDeployment(name, ns);
                break;
            case 'service':
                await config.kubeConfig.makeApiClient(CoreV1Api).deleteNamespacedService(name, ns);
                break;
            case 'pod':
                await config.kubeConfig.makeApiClient(CoreV1Api).deleteNamespacedPod(name, ns);
                break;
            default:
